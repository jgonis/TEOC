 ...
 * method for loading a program (HACK file) into the ROM and another method for
 * running the program which is currently in them ROM. Also gives read access to
 * the memory.
 */
public class CPU {
	 ...
	 */
	public CPU(RAM ram, ROM rom, PointerAddressRegisterAdapter A, Register D, PointerAddressRegisterAdapter PC, ALU alu,
			Bus bus) {
		this.M = ram;
		this.rom = rom;
		this.A = A;
	...
	protected void computeExp(short instruction) throws ProgramException {
		...
		try {
			alu.setCommand(assemblerTranslator.getExpByCode((short) (instruction & 0xffc0)), zd, nd, zm, nm, f, no);

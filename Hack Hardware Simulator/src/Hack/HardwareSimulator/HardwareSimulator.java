 ...
 *
 * Recognizes the following variables: time - the number of tick-tocks that
 * passed since the program started running. if clock is up, adds a '+' (String)
 * - READ ONLY Input/Output/Internal pin name
 *
 * Recognizes the following commands: load <gate name> - loads the given gate
 * (from an HDL file) into the simulator (the file name should not contain the
 * path and the .HDL extension). tick - Clock goes up (internal state of clocked
 * gates changes). tock - Clock goes down (outputs of clocked gates are
 * modified). eval - propagate all the input values of the gate and re-compute
 * all outputs of the gate.
 */
public class HardwareSimulator extends HackSimulator
		implements TextFileEventListener, GateErrorEventListener, DirtyGateListener {
	...
	public static String getFullPinName(String name, byte[] subBus) {
		...
		if (subBus != null && !name.equals(CompositeGateClass.TRUE_NODE_INFO.name)
		...
		if (command[0].equals(COMMAND_TICK)) {
			...
			try {
				...
				if (command[1].indexOf("/") >= 0)
					throw new CommandException("The gate name must not contain path specification", command);
	...
	/**
	 * Called when an error occured in a gate's component. The event contains
	 * the source object and the error message.
	 */
			...
			if (node != null)
				...
				if (gateName != null) {
					...
					if (guiChip != null) {
						...
						throw new VariableException("No such built-in chip used", gateName);
					}
				}
	...
	private int getVarIndex(String varName) throws VariableException {
		...
		if (varName.endsWith("[]"))
			...
			try {
				index = Integer.parseInt(varName.substring(varName.indexOf("[") + 1, varName.indexOf("]")));
			} catch (Exception nfe) {
			...
			if (node != null) {
				...
				if (type != GateClass.INPUT_PIN_TYPE)
					...
					if (!isLegalWidth(varName, numValue))
						throw new VariableException(value + " doesn't fit in the pin's width", varName);
					else
						node.set(numValue);
				}
			} else {

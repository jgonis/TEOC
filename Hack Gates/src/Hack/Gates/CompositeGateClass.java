...
public class CompositeGateClass extends GateClass {
	...
	/**
	 * Constructs a new CompositeGateClass with the given gate name and the
	 * HDLTokenizer input which is positioned just after the PARTS: declaration.
	...
	public CompositeGateClass(String gateName, HDLTokenizer input, PinInfo[] inputPinsInfo, PinInfo[] outputPinsInfo)
		...
		int counter = 0;
		for (Object element : topologicalOrder) {
			if (element instanceof Integer) {
				partsOrder[counter++] = ((Integer) element).intValue();
			}
		}

		// for each input pin, check if there is a path in the graph to an
		// output pin
		...
		// for each output pin, check if there is a path in the graph from any
		// input pin
		// (actually from the "master input", which connects to all inputs) to
		// this output pin.
	...
	private void addConnection(HDLTokenizer input, int partNumber, String partName, String fullLeftName,
		...
		byte leftWidth = (leftSubBus == null ? leftPinInfo.width : (byte) ((leftSubBus[1] - leftSubBus[0]) + 1));
		...
		if (rightName.equals(CompositeGateClass.TRUE_NODE_INFO.name)) {
			...
			// find right pin's info. If doesn't exist, create it as an internal
			// pin.
		...
		if (selfFittingWidth) {
			...
			rightWidth = (rightSubBus == null ? rightPinInfo.width : (rightSubBus[1] - rightSubBus[0]) + 1);
		}
		...
		// make sure that an internal pin is only fed once by a part's output
		// pin
		...
		Connection connection = new Connection(connectionType, rightNumber, partNumber, leftName, rightSubBus,
				leftSubBus);
	 ...
	 * containing the part's numbers. 2. Input, Output, Internal and special
	 * nodes, represented with their PinInfo objects. 3. One "master part" node
	 * that connects to all the inernal parts, represented with the partsList
	 * vector. 4. One "master output" node that all output nodes connect to,
	 * represented with the outputPinsInfo array. 5. One "master input" node
	 * that connects to all input nodes, represented with the inputPinsInfo
	 * array. Edges are not created between inetrnal nodes and clocked part
	 * inputs.
	 */
	private Graph createConnectionsGraph() {
		...
		for (PinInfo element : outputPinsInfo) {
			graph.addEdge(element, outputPinsInfo);
		}

		// connect the "master input" node to all input pins
		for (PinInfo element : inputPinsInfo) {
			graph.addEdge(inputPinsInfo, element);
		}
	...
	public Gate newInstance() throws InstantiationException {
		...
		// Create instances (Gates) from all parts in the parts list (which are
		// GateClasses).
		...
		// Creates another parts array in which the parts are sorted in
		// topological order.
		...
		// First scan: creates internal Nodes (or SubNodes) and their
		// connections to
		// their source part nodes. Also creates the connections between gate's
		// input or putput nodes and part's input nodes and between part's
		// output nodes and gate's
		// output nodes.
		...
		while (connectionIter.hasNext()) {
			...
			switch (connection.getType()) {
			case Connection.FROM_INPUT:
				connectGateToPart(inputNodes[connection.getGatePinNumber()], gateSubBus, partNode, partSubBus);
				break;

			case Connection.TO_OUTPUT:
				connectGateToPart(partNode, partSubBus, outputNodes[connection.getGatePinNumber()], gateSubBus);
				break;
		...
		// Second scan: Creates the connections between internal nodes or true
		// node
		...
		// If the clock special node appears in this gate, Add a dirty gate
		// adapter

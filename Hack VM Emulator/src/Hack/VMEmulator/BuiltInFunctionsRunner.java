 ...
 * pause and call VM code that the user may debug and step through.
 */
public class BuiltInFunctionsRunner implements Runnable {
	 ...
	 * call-backs and returns to the given CPU and load built-in vm code from
	 * the given directory.
	 */
	 ...
	 * name and number of parameters (the length of the params array). If found
	 * - tells the built-in code runner thread to call the named built-in
	 * function with the given params. Throws a ProgramException if no built-in
	...
	public void callBuiltInFunction(String functionName, short[] params) throws ProgramException {
		...
		try {
			...
			throw new ProgramException(
					"Can't find " + className + ".vm or a built-in implementation for class " + className);
		}
		...
		if (!found) {
			throw new ProgramException(
					"Built-in implementation for " + className + " is not a subclass of BuiltInVMClass");
		}
		...
		Method functionObject;
		try {
			functionObject = implementingClass.getDeclaredMethod(methodName, paramsClasses);
		} catch (NoSuchMethodException nsme) {
			throw new ProgramException("Can't find " + className + ".vm or a built-in implementation for function "

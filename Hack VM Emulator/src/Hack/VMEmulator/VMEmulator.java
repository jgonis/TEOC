 ...
 *
 * Recognizes the following variables: RAM[i] - the contents of the RAM at
 * location i (short) sp - the value of the stack pointer (short) local - the
 * address of the Local segment (short) local[i] - the contents of the i'th
 * element in the Local segment (short) argument - the address of the Argument
 * segment (short) argument[i] - the contents of the i'th element in the
 * Argument segment (short) this - the address of the This segment (short)
 * this[i] - the contents of the i'th element in the This segment (short) that -
 * the address of the That segment (short) that[i] - the contents of the i'th
 * element in the That segment (short 0..7) temp[i] - the contents of the i'th
 * element in the Temp segment (short) currentFunction - the name of the current
...
public class VMEmulator extends HackSimulator implements ComputerPartErrorEventListener {
	...
	public VMEmulator() {
		...
		AbsolutePointedMemorySegment stackSegment = new AbsolutePointedMemorySegment(ram, null);
		TrimmedAbsoluteMemorySegment workingStackSegment = new TrimmedAbsoluteMemorySegment(ram, null);
		MemorySegment staticSegment = new MemorySegment(ram, null);
		MemorySegment localSegment = new MemorySegment(ram, null);
		MemorySegment argSegment = new MemorySegment(ram, null);
		MemorySegment thisSegment = new MemorySegment(ram, null);
		MemorySegment thatSegment = new MemorySegment(ram, null);
		MemorySegment tempSegment = new MemorySegment(ram, null);

		stackSegment.reset();
		stackSegment.addErrorListener(this);
		stackSegment.setEnabledRange(Definitions.STACK_START_ADDRESS, Definitions.STACK_END_ADDRESS, true);

		workingStackSegment.reset();
		workingStackSegment.addErrorListener(this);
		workingStackSegment.setEnabledRange(Definitions.STACK_START_ADDRESS, Definitions.STACK_END_ADDRESS, true);
		...
		cpu = new CPU(program, ram, callStack, calculator, bus, stackSegment, workingStackSegment, staticSegment,
				localSegment, argSegment, thisSegment, thatSegment, tempSegment, INITIAL_BUILTIN_DIR);
	...
	public VMEmulator(VMEmulatorGUI gui) {
		...
		// assignes the memory segments
		AbsolutePointedMemorySegment stackSegment = new AbsolutePointedMemorySegment(ram, gui.getStack());
		TrimmedAbsoluteMemorySegment workingStackSegment = new TrimmedAbsoluteMemorySegment(ram, gui.getWorkingStack());
		MemorySegment staticSegment = new MemorySegment(ram, gui.getStaticSegment());
		MemorySegment localSegment = new MemorySegment(ram, gui.getLocalSegment());
		MemorySegment argSegment = new MemorySegment(ram, gui.getArgSegment());
		MemorySegment thisSegment = new MemorySegment(ram, gui.getThisSegment());
		MemorySegment thatSegment = new MemorySegment(ram, gui.getThatSegment());
		MemorySegment tempSegment = new MemorySegment(ram, gui.getTempSegment());
	...
	public String getValue(String varName) throws VariableException {
		if (varName.equals(VAR_LOCAL))
			...
			return String.valueOf(
					cpu.getCallStack().getTopFunction() + "." + cpu.getCurrentInstruction().getIndexInFunction());

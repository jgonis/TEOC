...
public class CPU {
	...
	// A mapping from memory segment codes to the MemorySegment objects (not
	// including stack).
	...
	/**
	 * Constructs the CPU with given program, RAM, call stack, bus, stack and
	 * other memory segments.
	 */
	public CPU(VMProgram program, RAM ram, CallStack callStack, Calculator calculator, Bus bus,
			AbsolutePointedMemorySegment stackSegment, TrimmedAbsoluteMemorySegment workingStackSegment,
			MemorySegment staticSegment, MemorySegment localSegment, MemorySegment argSegment,
			MemorySegment thisSegment, MemorySegment thatSegment, MemorySegment tempSegment, File builtInDir) {
	...
	public void boot() {
		...
		localSegment.setEnabledRange(Definitions.STACK_START_ADDRESS, Definitions.STACK_END_ADDRESS, true);
		argSegment.setEnabledRange(Definitions.STACK_START_ADDRESS, Definitions.STACK_END_ADDRESS, true);
		thisSegment.setEnabledRange(Definitions.HEAP_START_ADDRESS, Definitions.HEAP_END_ADDRESS, true);
		thatSegment.setEnabledRange(Definitions.HEAP_START_ADDRESS, Definitions.SCREEN_END_ADDRESS, true);
		staticSegment.setStartAddress(Definitions.VAR_START_ADDRESS);
		staticSegment.setEnabledRange(Definitions.VAR_START_ADDRESS, Definitions.VAR_END_ADDRESS - 1, true);
	...
	public void executeInstruction() throws ProgramException {
		...
		case HVMInstructionSet.CALL_CODE:
			callFunction(currentInstruction.getArg0(), currentInstruction.getArg1(), currentInstruction.getStringArg(),
					false);
			break;
		}
	}
	...
	public void function(short numberOfLocals) throws ProgramException {
		...
		// disable non relevant range of the local segment - enable only the
		// number
	 ...
	 * needed because there is no good way to use the stop button to stop an
	 * infinite loop in a built-in jack class. A message containing information
	...
	public void returnFromFunction() throws ProgramException {
		...
		if (stackSegment.getValueAt(Definitions.LOCAL_POINTER_ADDRESS) == 0) {
			throw new ProgramException("Nowhere to return to in " + getCallStack().getTopFunction() + "."
		...
		bus.send(ram, Definitions.LOCAL_POINTER_ADDRESS, ram, Definitions.R13_ADDRESS); // R13
																						// =
																						// lcl
		bus.send(stackSegment, stackSegment.getValueAt(Definitions.LOCAL_POINTER_ADDRESS) - 5, ram,
				Definitions.R14_ADDRESS); // R14 = return address
		bus.send(stackSegment, getSP() - 1, stackSegment, ram.getValueAt(Definitions.ARG_POINTER_ADDRESS)); // *arg
																											// =
																											// return
																											// value
		setSP((short) (ram.getValueAt(Definitions.ARG_POINTER_ADDRESS) + 1)); // SP
																				// =
																				// arg
																				// +
																				// 1
		bus.send(stackSegment, ram.getValueAt(Definitions.R13_ADDRESS) - 1, ram, Definitions.THAT_POINTER_ADDRESS); // that
																													// =
																													// *(R13
																													// -
																													// 1)
		bus.send(stackSegment, ram.getValueAt(Definitions.R13_ADDRESS) - 2, ram, Definitions.THIS_POINTER_ADDRESS); // this
																													// =
																													// *(R13
																													// -
																													// 2)
		bus.send(stackSegment, ram.getValueAt(Definitions.R13_ADDRESS) - 3, ram, Definitions.ARG_POINTER_ADDRESS); // arg
																													// =
																													// *(R13
																													// -
																													// 3)
		bus.send(stackSegment, ram.getValueAt(Definitions.R13_ADDRESS) - 4, ram, Definitions.LOCAL_POINTER_ADDRESS); // lcl
																														// =
																														// *(R13
																														// -
																														// 4)
		...
		if (stackFrames.size() > 0) {
			...
			int frameAddress = stackFrames.lastElement().intValue();
			...
			localSegment.setEnabledRange(Math.max(localSegment.getStartAddress(), Definitions.STACK_START_ADDRESS),
					frameAddress - 1, true);

			// enable in the arg segment only the number of args that were sent
			// to the function
			// that we returned to.
			argSegment.setEnabledRange(argSegment.getStartAddress(), localSegment.getStartAddress() - 6, true);

			// enable this, that according to their retrieved pointers
			thisSegment.setEnabledRange(Math.max(thisSegment.getStartAddress(), Definitions.HEAP_START_ADDRESS),
					Definitions.HEAP_END_ADDRESS, true);
			thatSegment.setEnabledRange(Math.max(thatSegment.getStartAddress(), Definitions.HEAP_START_ADDRESS),
					Definitions.SCREEN_END_ADDRESS, true);
		} /*
			 * else { error("Nowhere to return to"); }
		...
		if (returnAddress == VMProgram.BUILTIN_FUNCTION_ADDRESS) {
			...
			program.setPC((short) (returnAddress - 1)); // set previousPC
														// currectly
	...
	/**
	 * Sets the static segment range according to the the given function (file)
	 * name.
	 */
